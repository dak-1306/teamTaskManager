...
function getWeekInfo(year: number, week: number) {
...
function normalizeWeek(tasksByDay: any[], year: number, week: number) {
...
export default function TasksLineChartByWeek({ tasksByDay = [] }: { tasksByDay?: any[] }) {
  ...
  return (
    <div className="w-full">
      ...
      <div style={{ width: "100%", height: 400 }}>
        <ChartContainer
          config={{ created: { color: "#3b82f6" }, completed: { color: "#22c55e" } }}
          id="tasks-line-week"
        >
          <LineChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="date" />
            <YAxis allowDecimals={false} tickCount={6} domain={[0, "dataMax + 2"]} />
            <ChartTooltip />
            <Legend />
            <Line type="monotone" dataKey="created" stroke="#3b82f6" strokeWidth={3} dot={{ r: 4 }} />
